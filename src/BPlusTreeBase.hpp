...
{
    EntryItem* itm = new EntryItem(item.first, item.second);
    Node* node = get_root();
    Key key = get_entry_key(itm);
    Node* ins;
    insert_req(node, nullptr, itm, ins);
    return iterator(ins, key);
}
...
typename BPlusTreeBase<Key,T>::iterator BPlusTreeBase<Key,T>::begin()
{
    if(!size())
        return end();
    Node* node = min_node();
    return iterator(node,get_entry_key(node->first_child()));
}
...
bool BPlusTreeBase<Key,T>::erase_req(Node* node, Node* parent, Key& key)
{
    ...
    nodeRight = parent->find_prev(key);
